...
$$\mathbb{E}[(\int_t^T e^{-\rho (s-t)} \cdot \frac {c_s^{1-\gamma}} {1-\gamma} \cdot ds + e^{-\rho (T-t)} \cdot B \cdot \frac{W_T^{1-\gamma}} {1-\gamma}) \mid W_t]$$
where $\rho \geq 0$ is the utility discount rate, $B$ is bequest utility factor
\item We can solve this problem for arbitrary $B$ but Merton considers $B = 0$
\end{itemize}
\end{frame}
