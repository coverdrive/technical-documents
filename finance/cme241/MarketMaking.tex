...
\item To develop intuition for Indifference Prices, consider a simple case where the market-maker doesn't supply any bids or asks. Then,
...
$$Q_t^{(b)} = S_t + (1-2I_t) \frac {\gamma \sigma^2(T-t)} {2}$$
$$Q_t^{(a)} = S_t + (-1-2I_t) \frac {\gamma \sigma^2(T-t)} {2}$$
$$Q_t^{(m)} = S_t - I_t \gamma \sigma^2(T-t)$$
$$Q_t^{(a)} - Q_t^{(b)} = \gamma \sigma^2(T-t)$$
\end{itemize}
\end{frame}
