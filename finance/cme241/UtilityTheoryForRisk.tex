...
\begin{itemize}
\item Game where your payoff is based on outcome of a fair coin
...
\begin{itemize}
\item So the amount we'd pay to consume a random outcome involves
\begin{itemize}
\item The Mean of the outcome (often called {\bf Reward})
\item The Variance of the outcome ({\bf Risk})
\item Our personal degree of {\bf Risk-Aversion}
\end{itemize}
\item More generally, we want a ``valuation formula'' that combines:
\begin{itemize}
\item The entire probability distribution of the random outcome
\item A function of the outcome that captures our Risk-Aversion
\end{itemize}
\item This function of the outcome is called {\em Utility} (denoted $U$)
\item It relates to a fundamental concept called {\bf Utility of Consumption}
\item We will illustrate this economic concept with a real-life example
\end{itemize}
\end{frame}
...
\item Since $\mathbb{E}[U(x)] = U(x_{CE})$, the above two expressions are equal. Hence,
$$U'(\bar{x}) \cdot (x_{CE} - \bar{x}) = \frac 1 2 \cdot U''(\bar{x}) \cdot \sigma_x^2$$
\end{itemize}
\end{frame}
...
$$\pi_A = \bar{x} - x_{CE} = - \frac 1 2 \cdot \frac {U''(\bar{x})} {U'(\bar{x})} \cdot \sigma_x^2$$
\item We refer to function A(x) = $-\frac {U''(x)} {U'(x)}$ as the {\bf Absolute Risk-Aversion}
$$\pi_A = \frac 1 2 \cdot A(\bar{x}) \cdot \sigma_x^2$$
...
$$\pi_R = \frac {\pi_A} {\bar{x}} = - \frac 1 2 \cdot \frac {U''(\bar{x}) \cdot \bar{x}} {U'(\bar{x})} \cdot \frac {\sigma_x^2} {\bar{x}^2} = - \frac 1 2 \cdot \frac {U''(\bar{x}) \cdot \bar{x}} {U'(\bar{x})} \cdot \sigma_{\frac x {\bar{x}}}^2$$
\item We refer to function R(x) = $-\frac {U''(x) \cdot x} {U'(x)}$ as the {\bf Relative Risk-Aversion}
$$\pi_R = \frac 1 2 \cdot R(\bar{x}) \cdot \sigma_{\frac x {\bar{x}}}^2$$
\end{itemize}
\end{frame}
