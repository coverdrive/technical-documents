...
\begin{equation}
\sum_i \phi(s_i,a_i) \cdot (\bm{\phi}(s_i,a_i) \cdot \bm{w}^* - (r_i + \gamma \cdot \bm{\phi}(s'_i, \pi_D(s'_i)) \cdot \bm{w}^*)) = 0
...
\begin{equation}
\sum_i \bm{\phi}(s_i) \cdot (\bm{\phi}(s_i) \cdot \bm{w}^* - \gamma \cdot \hat{Q}(s'_i, \pi_D(s'_i); \bm{w}^*)) = 0
\label{eq:customized-lspi-loss-semi-gradient}
\end{equation}
\item We need to consider two cases for the term $\hat{Q}(s'_i, \pi_D(s'_i); \bm{w}^*)$
\begin{itemize} 
\item  $C1$: If $s'_i$ is non-terminal and $\pi_D(s'_i) = c$ (i.e., $\bm{\phi}(s'_i) \cdot \bm{w} \geq g(s'_i)$):\\
Substitute $\bm{\phi}(s'_i) \cdot \bm{w}^*$ for $\hat{Q}(s'_i,\pi_D(s'_i); \bm{w}^*)$ in Equation \eqref{eq:customized-lspi-loss-semi-gradient}
\item $C2$: If $s'_i$ is a terminal state or $\pi_D(s'_i) = e$ (i.e., $g(s'_i) > \bm{\phi}(s'_i) \cdot \bm{w}$):\\
Substitute $g(s'_i)$ for $\hat{Q}(s'_i,\pi_D(s'_i); \bm{w}^*)$ in Equation \eqref{eq:customized-lspi-loss-semi-gradient}
\end{itemize}
\item So we can rewrite Equation \eqref{eq:customized-lspi-loss-semi-gradient} as:
$$\sum_{i|C1} \bm{\phi}(s_i) \cdot (\bm{\phi}(s_i) \cdot \bm{w}^* - \gamma \cdot \bm{\phi}(s'_i) \cdot \bm{w}^*) + \sum_{i|C2} \bm{\phi}(s_i) \cdot (\bm{\phi}(s_i) \cdot \bm{w}^* -  \gamma \cdot g(s'_i)) = 0$$
\item Factoring out $\bm{w}^*$ and using indicator notation for cases $C1, C2$:
$$(\sum_i \bm{\phi}(s_i) \otimes (\bm{\phi}(s_i) - \mathbb{I}_{C1} \cdot \gamma \cdot \bm{\phi}(s'_i))) \cdot \bm{w}^*= \gamma \cdot \sum_i  \mathbb{I}_{C2} \cdot \bm{\phi}(s_i) \cdot g(s'_i)$$
...
$$\bm{A} = \sum_i \bm{\phi}(s_i) \otimes (\bm{\phi}(s_i) - \mathbb{I}_{\bm{\phi}(s'_i) \cdot \bm{w} \geq g(s'_i)} \cdot \gamma \cdot \bm{\phi}(s'_i))$$
$$\bm{b} = \gamma \cdot \sum_i \mathbb{I}_{g(s'_i)  > \bm{\phi}(s'_i) \cdot  \bm{w}} \cdot \bm{\phi}(s_i) \cdot  g(s'_i)$$
\item $m \times m$ Matrix $\bm{A}$ is accumulated at each atomic experience $(s_i,s'_i)$ as:
$$\bm{A} \leftarrow \bm{A} + \bm{\phi}(s_i) \otimes (\bm{\phi}(s_i) -  \mathbb{I}_{\bm{\phi}(s'_i) \cdot \bm{w} \geq g(s'_i)} \cdot \gamma \cdot \bm{\phi}(s'_i))$$
\item $m$-Vector $\bm{b}$ is accumulated at each atomic experience $(s_i, s'_i)$ as:
$$\bm{b} \leftarrow \bm{b} + \gamma  \cdot \mathbb{I}_{g(s'_i) > \bm{\phi}(s'_i) \cdot \bm{w}} \cdot \bm{\phi}(s_i) \cdot g(s'_i)$$
