...
\begin{frame}
\frametitle{Pricing and Hedging Problem Statement}
\begin{itemize}
...
\item We want to determine the valuation (pricing) of the incremental position $D'$, as well as the hedging strategy for $D'$
\item Denote the Optimal Value Function at time $t$ as $V_t^* : \mathcal{S}_t \rightarrow \mathbb{R}$
\item The Valuation of $D'$ is based on the breakeven Optimal Value Function for $D \cup D'$ relative to $D$, i.e., it's the value $x$ such that
