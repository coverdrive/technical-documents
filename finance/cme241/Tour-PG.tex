...
\title[Policy Gradient Chapter]{A Guided Tour of \href{http://stanford.edu/~ashlearn/RLForFinanceBook/book.pdf}{\underline{\textcolor{yellow}{Chapter 12}}}: \\ Policy Gradient Algorithms} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\item DPG expressed as Expected Gradient of Q-Value Function
...
\item Is Off-Policy: Target policy $\pi_D$ learnt, behavior policy $\mu$ is exploratory
\item Core Idea: Instead of greedy policy improvement for continuous action spaces, move policy in the direction of gradient of Q-Value Function
...
\end{theorem}
\pause
In practice, we use critic $Q(s,a;\bm{w})$ and keep updating both $\bm{w}$ and $\bm{\theta}$:
\pause
