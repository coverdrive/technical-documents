...
\date{Winter 2026} % Date, can be changed to a custom date
...
\item Leading ICME's {\em Mathematical and Computational Finance} program
\item Technical mentor to ICME students
\item Teaching faculty at {\em HAI Institute}
...
\item Currently EVP AI at o9 (Enterprise AI software company)
\item Teaching experience: Mathematics, Comp Sci, AI, Finance
\end{itemize}
\end{frame}
...
\end{itemize}
\item Register for the \href{https://edstem.org/us/courses/90295}{\underline{\textcolor{blue}{course on Ed Discussion}}}
\item Install Python 3 and supporting IDE/tools (eg: VS Code, Jupyter)
...
\item {\em Fork} this repo and get set up to use this code in assignments
\item Create separate directories for each assignment for CA \href{mailto:cyc2152@stanford.edu}{\underline{\textcolor{blue}{Catherine Chen}}} to review
\item Send Catherine your forked repo URL and {\em git push} by due dates
\end{itemize}
\end{frame}
...
\pause
\begin{itemize}[<+->]
\item Lectures: Wed \& Fri 4:30pm-5:50pm at Gates B12
\item Office Hours:
\begin{enumerate}
	\item  Ashwin: 2:30pm-4:00pm Fri (or by appointment) in ICME Mezzanine, room M05 (within Huang Engg Bldg)
	\item  Catherine: poll will be sent out on Ed (or by appointment)
\end{enumerate}
\item Course Web Site: \href{http://cme241.stanford.edu}{\underline{\textcolor{blue}{cme241.stanford.edu}}}
\item Ask Questions and engage in Discussions on \href{https://edstem.org/us/courses/90295}{\underline{\textcolor{blue}{Ed Discussion}}}
\item Note: This course is listed as CME 241 as well as MS\&E 346
\item Any accommodations: Please reach out to me or Catherine as soon as possible
\end{itemize}
\end{frame}
...
\item Slides available for each lecture (``guided tour'' of respective chapter)
...
\begin{frame}
	...
	\begin{itemize}[<+->]
		\item Breakdown
		\begin{itemize}
			\item 45\% {\em Group Assignments} (Mix of Math and Programming)
			\item 50\% {\em Course Project} (Idea Creativity, Proof-of-Concept, Practical Problem, Presentation)
                \item 5\% {\em Attendance} (18/20 Minimum Lecture Attendance)
		\end{itemize}
	
		\item Assignments
		\begin{itemize}
			\item 3 assignments total, each worth 15\% and graded based on accuracy
                \item Can be completed in groups of up to 3 (single repository)
			\item 3 free late days, -10\% for each late day thereafter
		\end{itemize}
	
		\item Course Project
		\begin{itemize}
			\item Design a problem specialization within the space of {\em Personal Finance}
			\item Group size of at most 3, can be same group as assignments
			\item Delivery in 3 phases as described in detail in this \href{https://github.com/coverdrive/technical-documents/blob/master/finance/cme241/Project-Winter2026.pdf}{\underline{\textcolor{blue}{project guidance doc}}}
		\end{itemize}
	\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Stanford Honor Code - Assignments and Project}
\pause
\begin{itemize}[<+->]
\item You can discuss approaches among your group members, not with other students outside of your group
\item Each group member should be responsible for a subset (indicate which question you do on the assignment)
\item Writing answers/code should be your own (don't copy/paste)
\item You can invoke the core modules I have written (as instructed), and can use LLMs
\end{itemize}
\end{frame}
...
\item RL Agents: Bots that act or behave more optimal than humans
...
\item But it doesn't work for American options with complex payoffs
